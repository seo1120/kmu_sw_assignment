...
// turn on the LED if the distance is between dist_min and dist_max
    ...
    if (190<= dist_raw && dist_raw <= 210){
       analogWrite(PIN_LED, 0);
    }
     else if (dist_raw >= 150 && dist_raw <= 250){
      analogWrite(PIN_LED, 127);
    }
    else if (100<=dist_raw && dist_raw<=300){
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  before_dist_raw = reading;
  if(reading < dist_min || reading > dist_max) reading = before_dist_raw; // return 0 when out of range.
  return reading;
